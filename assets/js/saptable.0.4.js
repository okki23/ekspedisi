/*! 
 * jQuery sapTable v0.4 - 09/03/2017
 * Copyright (c) 2017-2018 Sapta Marga Helmy (http://www.saptable.com)
...
(function ($) {
    ...
    $.fn.sapTable = function (options) {
        ...
        $(this).each(function (nmr, vle) {
            ...
            var gData = $.ajax({type: "GET", url: url, async: false}).responseText;
            var data = JSON.parse(gData);
...
// Set Display Records
            $.fn.showRecordTable(options.countLimit, vle.id, url);
            $.fn.getColumn(vle.id);
        });
        return $(this);
    };
    ...
    $.fn.refresh_sapTable = function (options) {
        ...
        var gData = $.ajax({type: "GET", url: options.url, async: false}).responseText;
        var data = JSON.parse(gData);
    ...
    $.fn.search_proses = function (params, idElem, url) {
        ...
        var options = dinamic_options[idElem];
        formatters = options.formatters;

        var gData = $.ajax({type: "GET", url: search_url, async: false}).responseText;
        var data = JSON.parse(gData);
