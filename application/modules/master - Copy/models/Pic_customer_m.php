...
class Pic_customer_m extends Parent_Model {
    ...
    protected $_table_name = 'm_pic_customer';
    ...
    public function get_data_pic($dt){
     ...
     public function pic_customer_json (){
        $this->db->select('a.*,b.position_name');
        $this->db->from('m_pic_customer a');
        $this->db->join('m_position b','b.id=a.customer_pic_position','left');
        return $this->db->get();
        /*SELECT a.customer_pic_name, b.position_name, a.customer_pic_phone, a.customer_pic_email FROM `m_pic_customer` LEFT JOIN `m_position` `b` ON `b`.`id`=`a`.`customer_pic_position` WHERE `a`.`code_customer*/
    }
}
